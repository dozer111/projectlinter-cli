...
func (l *Linter) Sets() ([]rules.Set, error) {
	...
	sets := []rules.Set{
		...
		dependency.NewPHPDependencySet(
			l.pathProvider,
			substituteLibraryConfigs, // go:generate
			bumpLibraryConfigs,       // go:generate
		),
		NewMakefileSet(
			l.pathProvider,
			sources,
		...
		common_sets.NewEditorConfig(
			l.pathProvider,
			sources,
			"https://your_github.com/projects/your_php_library_template/browse/.editorconfig",
		),
	}
